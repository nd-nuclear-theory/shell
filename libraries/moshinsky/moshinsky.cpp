...
double RacahReductionFactorFirstSystemGT(
    ...
    const HalfInt& J2, const HalfInt& lambda
  )
// Prefactor for first-system operator in Racah two-system 
// reduction formula.
//
// Follows group theoretic conventions for RME normalization.
{
  return ParitySign(J1p+J2+J+lambda)*Hat(J1p)*Hat(J)
    *Wigner6J(J1p,Jp,J2,J,J1,lambda);
}
