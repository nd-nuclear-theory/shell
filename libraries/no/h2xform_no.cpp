/******************************************************************************
  ...
  1/30/13 (max): Update to mfdn_h2 class-based I/O.
...
// XformRadialMatriceslj -- container structure for all radial matrices in lj basis
// V. Constantinou (07/12/15) NO patch

struct XformRadialMatriceslj 
{
	RadialMatriceslj xform_p, xform_n;
...
void XformRadialMatriceslj::Initialize (const std::string& radial_basename_p, const std::string& radial_basename_n) 
{
	xform_p.Read(radial_basename_p + ".dat");
	xform_n.Read(radial_basename_n + ".dat");
}
	...
	)
{
	...
	for (int k1p = 0; k1p < dimension; ++k1p)
      ...
      // DBG: std::cout << "n11_max " << n11_max << std::endl;
      //std::cout << "n12_max " << n12_max << std::endl;
      //std::cout << "n21_max " << n21_max << std::endl;
      //std::cout << "n22_max " << n22_max << std::endl;
				...
				for (int n12 = 0; n12 <= n12_max; ++n12)
					for (int n21 = 0; n21 <= n21_max; ++n21)
						for (int n22 = 0; n22 <= n22_max; ++n22)
						{
							...
							//std::cout << "    Adding: " << s1.a1 << s1.a2 << s2.a1 << s2.a2 << std::endl;
      ...
      // DBG: if( (n11_max == -1) || (n12_max == -1) || (n21_max == -1) || (n22_max == -1) )
			// DBG:  std::cout << "    Resultant:"  << " calculated UNAS " << matrix_element << std::endl;
...
// TwoBodyMatrixSectorTransformlj uses radial files with j-dependence
// V. Constantinou (07/12/15) patch for NOs

void TwoBodyMatrixSectorTransformlj (const TwoBodyMatrixNljTzJP& source_matrix, TwoBodyMatrixNljTzJP& destination_matrix, 
				   /*const*/ XformRadialMatriceslj& xform_radial_matrices, 
				   int N1b_cut, int N2b_cut,
				   const SectorNljTzJP& sector
	)
{
	...
	for (int k1p = 0; k1p < dimension; ++k1p)
		for (int k2p = k1p; k2p < dimension; ++k2p)
		{
			...
			// define shorthands for n and l quantum numbers (for more readable coding below)
			// define j quantum number to be used below
			...
			HalfInt j11p = s1p.a1.Getj();
			HalfInt j12p = s1p.a2.Getj();
			HalfInt j21p = s2p.a1.Getj();
			HalfInt j22p = s2p.a2.Getj();
				...
				for (int n12 = 0; n12 <= n12_max; ++n12)
					for (int n21 = 0; n21 <= n21_max; ++n21)
						for (int n22 = 0; n22 <= n22_max; ++n22)
						{
							...
							{
								coefficient
									= xform_radial_matrices.xform_p.GetMatrixElementlj(n11,l11p,j11p,n11p,l11p,j11p)
									* xform_radial_matrices.xform_p.GetMatrixElementlj(n12,l12p,j12p,n12p,l12p,j12p)
									* xform_radial_matrices.xform_p.GetMatrixElementlj(n21,l21p,j21p,n21p,l21p,j21p)
									* xform_radial_matrices.xform_p.GetMatrixElementlj(n22,l22p,j22p,n22p,l22p,j22p);
							}
							else if (state_type == kNN)
							{
								coefficient
									= xform_radial_matrices.xform_n.GetMatrixElementlj(n11,l11p,j11p,n11p,l11p,j11p)
									* xform_radial_matrices.xform_n.GetMatrixElementlj(n12,l12p,j12p,n12p,l12p,j12p)
									* xform_radial_matrices.xform_n.GetMatrixElementlj(n21,l21p,j21p,n21p,l21p,j21p)
									* xform_radial_matrices.xform_n.GetMatrixElementlj(n22,l22p,j22p,n22p,l22p,j22p);
							}
							else if (state_type == kPN)
							{
								coefficient
...
// TransformationDescriptorlj just takes the private member variable XformRadialMatriceslj for data storage.
// This is a quick patch to make h2utils work with radial files with lj dependence
// V. Constantinou (07/12/15)

class TransformationDescriptorlj
{
public:
	TransformationDescriptorlj (const std::string& os_basename, 
				  const std::string& radial_basename_p, const std::string& radial_basename_n, 
				  int N1b_cut, int N2b_cut, 
				  int N1b, int N2b);
	void Initialize (int N1b_in, int N2b_in);
...
private:
	...
	// data
	XformRadialMatriceslj transformations_;
	TwoBodyBasisNljTzJP output_basis_;
	TwoBodyMatrixNljTzJP output_matrix_;
...
// Constructor for class TransformationDescriptorlj. 
// V. Constantinou (07/12/15)

TransformationDescriptorlj::TransformationDescriptorlj(const std::string& os_basename, 
				  const std::string& radial_basename_p, const std::string& radial_basename_n, 
				  int N1b_cut, int N2b_cut, 
				  int N1b, int N2b)
{
...
// TransformationDescriptorlj::Initialize (07/12/15)

void TransformationDescriptorlj::Initialize(int N1b_in, int N2b_in)
{
	// validation of cutoff
	if ( ( N1b_in < N1b_cut_ ) || ( N2b_in < N2b_cut_ ) )
	{
...
// TransformationDescriptorlj::TransformSector -- Change TwoBodyMatrixSectorTransform to TwoBodyMatrixSectorTransformlj
// V. Constantinou (07/12/15)

void TransformationDescriptorlj::TransformSector(const TwoBodyMatrixNljTzJP& input_matrix, const SectorNljTzJP& sector)
{
	...
	// generate matrix elements (if input exisits)
	if (input_matrix.HasSector(sector))
		TwoBodyMatrixSectorTransformlj(input_matrix, output_matrix_, transformations_, N1b_cut_, N2b_cut_, sector);
...
// ReadParameterslj -- same version as ReadParameters adapted to work with j-dependent radial files..

void ReadParameterslj(std::string& is_basename, std::vector<TransformationDescriptorlj>& transformations)
{
	...
	while ( std::getline(std::cin, line) )
	{
		...
		// generate transformation descriptor
		transformations.push_back(TransformationDescriptorlj(os_basename, radial_basename_p, radial_basename_n, N1b_cut, N2b_cut, N1b, N2b));
...
/*	std::string is_basename;
	...
	////////////////////////////////////////////////////////////////
	// transformation parameter setup  -- lj version -- uncomment for an lj run
	////////////////////////////////////////////////////////////////
	
	// read transformation parameters
	std::string is_basename;
	std::vector<TransformationDescriptorlj> transformations;
	ReadParameterslj(is_basename, transformations);

	// initialize master sector iteration truncation
	int N1b_master(0), N2b_master(0);
	// update master truncation to encompass output truncations
	for (std::vector<TransformationDescriptorlj>::iterator it = transformations.begin(); it != transformations.end(); ++it)
	{
		N1b_master = std::max( N1b_master, it->GetN1b() );
		N2b_master = std::max( N2b_master, it->GetN2b() );
	}
...
/*  for (std::vector<TransformationDescriptor>::iterator it = transformations.begin(); it != transformations.end(); ++it)
	...
	////////////////////////////////////////////////////////////////
	// output initialization -- lj version -- uncomment for lj run
	////////////////////////////////////////////////////////////////
...
/*	for (SectorNljTzJP sector(N1b_master,N2b_master); sector.InRange(); ++sector)
  ...
  std::cout << "End transforming the interaction" << std::endl << std::endl;

        std::cout << std::endl;*/

	////////////////////////////////////////////////////////////////
	// sector-by-sector transformation -- lj version -- uncomment for lj run
	////////////////////////////////////////////////////////////////
