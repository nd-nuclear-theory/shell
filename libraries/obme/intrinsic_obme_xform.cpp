...
namespace shell
{
  ...
  //
  // Indexing for M matrix defined in (31) of Navratil 2021 or its inverse,
  // broken into blocks by Delta_N.  We adopt a symmetric labeling
  //
  ...
  //    parity grade g.
  //
  //  - We must impose some truncation on the orbitals or, more generallly,
  //    orbital pairs included.  For one-body operators defined by a set of
  //    OBMEs truncated according to N<=Nmax, we must at least include N1<=Nmax
  //    and N2<=Nmax.
  //
  ...
  // block diagonal in the oscillator shell shift Delta_N=N1-N2 imposed by the
  // one-body operator.  If our one-body operator is defined on a given set of
  // orbitals with some one-body Nmax, we need to include at least the maximal
  // Delta_N=Nmax arising in the OBMEs for this operator, while additional
  // Delta_N subspaces are superfluous.
  //
  ...
  //
  //   These labels are redundant, as the parity may be deduced from Delta_N, as
  //   g~Delta_N.  However, the space is naturally defined by the (J0, g) of the
  //   one-body operator of interest, so we retain g as a label for
  //   transparency.
  //
  ...
  //
  //   These labels are redundant, as n2 may be deduced from the others for
  //   given Delta_N.
  //
  ...
  // ## Subspaces ##
  //
  // Within a full space defined by fixed (J0, g), and subject to
  // single-particle truncation Nmax, subspaces are ordered by:
  //
  //    * Increasing delta_N (delta_N=0,1,...,Nmax), either all even or all odd,
  //      as given by constraint delta_N~g.
  //
  ///////////////////////////////////////////////////////////////
  //
  // ## States ##
