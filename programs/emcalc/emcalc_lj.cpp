...
shell::RadialMatriceslj ReturnRadialIntegralslj (string& radial_integrals_partial_path, char& type,
				       int& lambda)
{
  ...
  if( !open_integrals_file )  
    ...
    {      
      radial_matrix.Read(radial_integrals_full_path);     
    }
...
void CalculateElectromagneticMatrixElementslj (PostProcessingParameters& input_file_parameters)
{  
  FILE * output_file;
  ...
  if( input_file_parameters.observable_type == 'E' &&
	   ...
	   (AllowedTriangle(header_info.ji,header_info.jf,HalfInt(lambda_value)) == 1) )
    {
      ...
      shell::RadialMatriceslj radial_matrix_protons = ReturnRadialIntegralslj (input_file_parameters.path_to_radial_integrals_file_protons,
								     input_file_parameters.observable_type,
								     input_file_parameters.lambda);

      shell::RadialMatriceslj radial_matrix_neutrons = ReturnRadialIntegralslj (input_file_parameters.path_to_radial_integrals_file_neutrons,
								     input_file_parameters.observable_type,
								     input_file_parameters.lambda);
      ...
      for (int i=0; i < (file_size/2); i++)
	{
	  if ( ((orbital_file[i].la+orbital_file[i].lb+lambda_value)%2==0) && 
	    ...
	    {    
	      radial_integral_p = radial_matrix_protons.GetMatrixElementlj(orbital_file[i].na,orbital_file[i].la,orbital_file[i].ja,orbital_file[i].nb,orbital_file[i].lb,orbital_file[i].jb);	     
	  ...
	  if ( ((orbital_file[i-(file_size/2)].la+orbital_file[i-(file_size/2)].lb+lambda_value)%2==0) && 
	    ...
	    {             
	      radial_integral_n = radial_matrix_neutrons.GetMatrixElementlj(orbital_file[i-(file_size/2)].na,orbital_file[i-(file_size/2)].la,orbital_file[i-(file_size/2)].ja,orbital_file[i-(file_size/2)].nb,orbital_file[i-(file_size/2)].lb,orbital_file[i-(file_size/2)].jb);     
...
{  
  ...
  CalculateElectromagneticMatrixElementslj(input); 
